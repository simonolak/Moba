...
qboolean Com_SafeMode(void) {
  ...
  for (i = 0; i < com_numConsoleLines; i++) {
    CmdTokenizeString(com_consoleLines[i]);
...
void ZFree(void *ptr) {
  ...
  if (!ptr) {
    Com_Error(ERR_DROP, "ZFree: NULL pointer");
  }

  block = (memblock_t *)((byte *)ptr - sizeof(memblock_t));
  if (block->id != ZONEID) {
    Com_Error(ERR_FATAL, "ZFree: freed a pointer without ZONEID");
  }
  if (block->tag == 0) {
    Com_Error(ERR_FATAL, "ZFree: freed a freed pointer");
  }
  // if static memory
  ...
  if (*(int *)((byte *)block + block->size - 4) != ZONEID) {
    Com_Error(ERR_FATAL, "ZFree: memory block wrote past end");
  }
...
=================
*/
sysEvent_t ComGetRealEvent(void) {
  int			r;
  sysEvent_t	ev;
...
=================
*/
sysEvent_t ComGetEvent(void) {
  if (com_pushedEventsHead > com_pushedEventsTail) {
    com_pushedEventsTail++;
    return com_pushedEvents[(com_pushedEventsTail - 1) & (MAX_PUSHED_EVENTS - 1)];
  }
  return ComGetRealEvent();
}
...
int ComEventLoop(void) {
  ...
  while (1) {
    ...
    default:
      ...
      Com_Error(ERR_FATAL, "ComEventLoop: bad event type %i", ev.evType);
      break;
    case SE_NONE:
      break;
    case SE_KEY:
    ...
    case SE_PACKET:
      ...
      if ((unsigned)buf.cursize > buf.maxsize) {
        Com_Printf("ComEventLoop: oversize packet\n");
        continue;
      }
...
void Com_Init(char *commandLine) {
  ...
  Cmd_AddCommand("quit", ComQuitFunc);
...
void ComWriteConfigToFile(const char *filename) {
  fileHandle_t	f;
...
/*
===============
ComWriteConfiguration

Writes key bindings and archived cvars to config file if modified
===============
*/
void ComWriteConfiguration(void) {
#ifndef DEDICATED // bk001204
  cvar_t	*fs;
#endif
  ...
  ComWriteConfigToFile("q3config.cfg");
...
void Field_CompleteCommand(field_t *field) {
  ...
  CmdTokenizeString(completionField->buffer);
