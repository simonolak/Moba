...
int MSGReadBits(msg_t *msg, int bits) {
  int			value;
  int			get;
  qboolean	sgn;
  int			i, nbits;
...
int MSG_ReadChar (msg_t *msg ) {
	...
	c = (signed char)MSGReadBits( msg, 8 );
...
int MSGReadByte(msg_t *msg) {
  int	c;

  c = (unsigned char)MSGReadBits(msg, 8);
  if (msg->readcount > msg->cursize) {
    c = -1;
  }
  return c;
...
int MSGReadLong(msg_t *msg) {
  int	c;
  c = MSGReadBits(msg, 32);
  if (msg->readcount > msg->cursize) {
    c = -1;
  }
  return c;
...
char *MSGReadString( msg_t *msg ) {
	static char	string[MAX_STRING_CHARS];
	int		l,c;
	
	l = 0;
	do {
		c = MSGReadByte(msg);		// use ReadByte so -1 is out of bounds
		if ( c == -1 || c == 0 ) {
			break;
		}
...
char *MSGReadStringLine(msg_t *msg) {
  static char	string[MAX_STRING_CHARS];
  int		l, c;
...
int	MSG_ReadDelta( msg_t *msg, int oldV, int bits ) {
	if ( MSGReadBits( msg, 1 ) ) {
		return MSGReadBits( msg, bits );
	}
	return oldV;
}
...
int	MSG_ReadDeltaKey( msg_t *msg, int key, int oldV, int bits ) {
	if ( MSGReadBits( msg, 1 ) ) {
		return MSGReadBits( msg, bits ) ^ (key & kbitmask[bits]);
	}
	return oldV;
}
...
float MSG_ReadDeltaKeyFloat( msg_t *msg, int key, float oldV ) {
	if ( MSGReadBits( msg, 1 ) ) {
		...
		*(int *)&newV = MSGReadBits( msg, 32 ) ^ key;
		return newV;
	}
	return oldV;
}
...
void MSG_ReadDeltaUsercmd( msg_t *msg, usercmd_t *from, usercmd_t *to ) {
	if ( MSGReadBits( msg, 1 ) ) {
		to->serverTime = from->serverTime + MSGReadBits( msg, 8 );
	} else {
		to->serverTime = MSGReadBits( msg, 32 );
	}
...
void MSGReadDeltaEntity(msg_t *msg, entityState_t *from, entityState_t *to,
  int number) {
  int			i, lc;
  int			numFields;
  netField_t	*field;
      ...
      if (field->bits == 0) {
        ...
        if (MSGReadBits(msg, 1) == 0) {
          *(float *)toF = 0.0f;
          ...
          if (MSGReadBits(msg, 1) == 0) {
            // integral float
            trunc = MSGReadBits(msg, FLOAT_INT_BITS);
        ...
        if (MSGReadBits(msg, 1) == 0) {
          *toF = 0;
          ...
          // integer
          *toF = MSGReadBits(msg, field->bits);
          if (print) {
