...
int		MSGReadByte (msg_t *sb);
int		MSG_ReadShort (msg_t *sb);
int		MSGReadLong (msg_t *sb);
float	MSG_ReadFloat (msg_t *sb);
char	*MSGReadString (msg_t *sb);
char	*MSG_ReadBigString (msg_t *sb);
char	*MSGReadStringLine (msg_t *sb);
...
// this can be used in place of either CbufAddText or Cbuf_InsertText

void CBufExecute (void);
// Pulls off \n terminated lines of text from the command buffer and sends
// them through CmdExecuteString.  Stops when the buffer is empty.
...
void	CmdTokenizeString( const char *text );
...
// expands value to a string and calls CvarSet
...
// called by CmdExecuteString when Cmd_Argv(0) doesn't match a known
// command.  Returns true if the command was a variable reference that
// was handled. (print or change)

void 	CvarWriteVariables( fileHandle_t f );
...
// FSCloseFile instead of fclose, otherwise the pak FILE would be improperly closed
...
void KeyWriteBindings( fileHandle_t f );
...
// SysMilliseconds should only be used for profiling purposes,
