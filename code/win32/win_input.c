...
void IN_DIMouse(int *mx, int *my) {
  ...
  for (;;) {
    ...
    case DIMOFS_BUTTON0:
      if (od.dwData & 0x80)
        SysQueEvent(od.dwTimeStamp, SE_KEY, K_MOUSE1, qtrue, 0, NULL);
      else
        SysQueEvent(od.dwTimeStamp, SE_KEY, K_MOUSE1, qfalse, 0, NULL);
      break;

    case DIMOFS_BUTTON1:
      if (od.dwData & 0x80)
        SysQueEvent(od.dwTimeStamp, SE_KEY, K_MOUSE2, qtrue, 0, NULL);
      else
        SysQueEvent(od.dwTimeStamp, SE_KEY, K_MOUSE2, qfalse, 0, NULL);
      break;

    case DIMOFS_BUTTON2:
      if (od.dwData & 0x80)
        SysQueEvent(od.dwTimeStamp, SE_KEY, K_MOUSE3, qtrue, 0, NULL);
      else
        SysQueEvent(od.dwTimeStamp, SE_KEY, K_MOUSE3, qfalse, 0, NULL);
      break;

    case DIMOFS_BUTTON3:
      if (od.dwData & 0x80)
        SysQueEvent(od.dwTimeStamp, SE_KEY, K_MOUSE4, qtrue, 0, NULL);
      else
        SysQueEvent(od.dwTimeStamp, SE_KEY, K_MOUSE4, qfalse, 0, NULL);
      break;
      // https://zerowing.idsoftware.com/bugzilla/show_bug.cgi?id=50
    case DIMOFS_Z:
      value = od.dwData;
      if (value == 0) {
        ...
        SysQueEvent(od.dwTimeStamp, SE_KEY, K_MWHEELDOWN, qtrue, 0, NULL);
        SysQueEvent(od.dwTimeStamp, SE_KEY, K_MWHEELDOWN, qfalse, 0, NULL);
      } else {
        SysQueEvent(od.dwTimeStamp, SE_KEY, K_MWHEELUP, qtrue, 0, NULL);
        SysQueEvent(od.dwTimeStamp, SE_KEY, K_MWHEELUP, qfalse, 0, NULL);
      }
      break;
    }
  }
...
void IN_MouseEvent(int mstate) {
  ...
  for (i = 0; i < 3; i++) {
    if ((mstate & (1 << i)) &&
      ...
      SysQueEvent(g_wv.sysMsgTime, SE_KEY, K_MOUSE1 + i, qtrue, 0, NULL);
    }

    if (!(mstate & (1 << i)) &&
      (s_wmv.oldButtonState & (1 << i))) {
      SysQueEvent(g_wv.sysMsgTime, SE_KEY, K_MOUSE1 + i, qfalse, 0, NULL);
    }
  }
...
void IN_MouseMove(void) {
  ...
  SysQueEvent(0, SE_MOUSE, mx, my, 0, NULL);
}
...
void IN_JoyMove(void) {
  ...
  for (i = 0; i < joy.jc.wNumButtons; i++) {
    if ((buttonstate & (1 << i)) && !(joy.oldbuttonstate & (1 << i))) {
      SysQueEvent(g_wv.sysMsgTime, SE_KEY, K_JOY1 + i, qtrue, 0, NULL);
    }
    if (!(buttonstate & (1 << i)) && (joy.oldbuttonstate & (1 << i))) {
      SysQueEvent(g_wv.sysMsgTime, SE_KEY, K_JOY1 + i, qfalse, 0, NULL);
    }
  }
  ...
  for (i = 0; i < 16; i++) {
    if ((povstate & (1 << i)) && !(joy.oldpovstate & (1 << i))) {
      SysQueEvent(g_wv.sysMsgTime, SE_KEY, joyDirectionKeys[i], qtrue, 0, NULL);
    }

    if (!(povstate & (1 << i)) && (joy.oldpovstate & (1 << i))) {
      SysQueEvent(g_wv.sysMsgTime, SE_KEY, joyDirectionKeys[i], qfalse, 0, NULL);
    }
  }
  ...
  if (joy.jc.wNumAxes >= 6) {
    ...
    if (x || y) {
      SysQueEvent(g_wv.sysMsgTime, SE_MOUSE, x, y, 0, NULL);
    }
  }
}
...
static void MIDI_NoteOff(int note) {
  ...
  SysQueEvent(g_wv.sysMsgTime, SE_KEY, qkey, qfalse, 0, NULL);
}
...
static void MIDI_NoteOn(int note, int velocity) {
  ...
  SysQueEvent(g_wv.sysMsgTime, SE_KEY, qkey, qtrue, 0, NULL);
}
...
static void CALLBACK MidiInProc(HMIDIIN hMidiIn, UINT uMsg, DWORD dwInstance,
  ...
  //	SysQueEvent( sys_msg_time, SE_KEY, wMsg, qtrue, 0, NULL );
}
