...
================
*/
void SysQueEvent(int time, sysEventType_t type, int value, int value2, int ptrLength, void *ptr) {
  sysEvent_t	*ev;
...
sysEvent_t SysGetEvent(void) {
  ...
  if (s) {
    ...
    SysQueEvent(0, SE_CONSOLE, 0, 0, len, b);
  }

  // check for network packets
  MSGInit(&netmsg, sys_packetReceived, sizeof(sys_packetReceived));
  if (SysGetPacket(&adr, &netmsg)) {
    netadr_t		*buf;
    int len;
    ...
    *buf = adr;
    ...
    SysQueEvent(0, SE_PACKET, 0, 0, len, buf);
  }
