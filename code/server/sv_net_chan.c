...
==============
*/
static void SVNetchanEncode(client_t *client, msg_t *msg) {
  ...
  reliableAcknowledge = MSGReadLong(msg);
...
static void SV_Netchan_Decode( client_t *client, msg_t *msg ) {
	...
	messageAcknowledge = MSGReadLong(msg);
	reliableAcknowledge = MSGReadLong(msg);
...
void SVNetchanTransmitNextFragment(client_t *client) {
  ...
  if (!client->netchan.unsentFragments) {
    ...
    if (!client->netchan_end_queue) {
      Com_Error(ERR_DROP, "netchan queue is not properly initialized in SVNetchanTransmitNextFragment\n");
    }
    ...
    if (client->netchan_start_queue) {
      ...
      SVNetchanEncode(client, &netbuf->msg);
      NetchanTransmit(&client->netchan, netbuf->msg.cursize, netbuf->msg.data);
      // pop from queue
