...
/*
===============
SVAddEntitiesVisibleFromPoint
===============
*/
static void SVAddEntitiesVisibleFromPoint(vec3_t origin, clientSnapshot_t *frame,
...
=============
*/
static void SVBuildClientSnapshot(client_t *client) {
  vec3_t						org;
  ...
  ps = SVGameClientNum(client - svs.clients);
  frame->ps = *ps;
  ...
  SVAddEntitiesVisibleFromPoint(org, frame, &entityNumbers, qfalse);
...
static int SVRateMsec(client_t *client, int messageSize) {
  int		rate;
  int		rateMsec;
...
void SVSendClientSnapshot(client_t *client) {
  ...
  MSGInit(&msg, msg_buf, sizeof(msg_buf));
...
void SVSendClientMessages(void) {
  ...
  for (i = 0, c = svs.clients; i < sv_maxclients->integer; i++, c++) {
    ...
    if (c->netchan.unsentFragments) {
      c->nextSnapshotTime = svs.time + SVRateMsec(c, c->netchan.unsentLength - c->netchan.unsentFragmentStart);
      SVNetchanTransmitNextFragment(c);
      continue;
    }

    // generate and send a new message
    SVSendClientSnapshot(c);
  }
}