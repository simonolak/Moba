...
void CL_Record_f( void ) {
  ...
  MSGInit (&buf, bufData, sizeof(bufData));
  MSGBitstream(&buf);
...
void CL_DemoCompleted( void ) {
  if (cl_timedemo && cl_timedemo->integer) {
    ...
    time = SysMilliseconds() - clc.timeDemoStart;
    if ( time > 0 ) {
...
void CL_DownloadsComplete( void ) {
  ...
  // if the gamestate was changed by calling ComEventLoop
...
void CL_DisconnectPacket( netadr_t from ) {
  ...
  if ( !NETCompareAdr( from, clc.netchan.remoteAddress ) ) {
    return;
  }
...
void CL_MotdPacket( netadr_t from ) {
  ...
  // if not from our server, ignore it
  if ( !NETCompareAdr( from, cls.updateServer ) ) {
    return;
  }
...
/*
=================
CLConnectionlessPacket
Responses to broadcasts, etc
=================
*/
void CLConnectionlessPacket(netadr_t from, msg_t *msg) {
  char	*s;
  char	*c;

  MSG_BeginReadingOOB(msg);
  MSGReadLong(msg);	// skip the -1

  s = MSGReadStringLine(msg);
...
=================
*/
void CLPacketEvent(netadr_t from, msg_t *msg) {
  int headerBytes;

  clc.lastPacketTime = cls.realtime;

  if (msg->cursize >= 4 && *(int *)msg->data == -1) {
    CLConnectionlessPacket(from, msg);
    return;
  }
...
int CL_ScaledMilliseconds(void) {
  return SysMilliseconds()*com_timescale->value;
}
...
static void CL_SetServerInfoByAddress(netadr_t from, const char *info, int ping) {
  ...
  for (i = 0; i < MAX_OTHER_SERVERS; i++) {
    if (NETCompareAdr(from, cls.localServers[i].adr)) {
      CL_SetServerInfo(&cls.localServers[i], info, ping);
    }
  }

  for (i = 0; i < MAX_OTHER_SERVERS; i++) {
    if (NETCompareAdr(from, cls.mplayerServers[i].adr)) {
      CL_SetServerInfo(&cls.mplayerServers[i], info, ping);
    }
  }

  for (i = 0; i < MAX_GLOBAL_SERVERS; i++) {
    if (NETCompareAdr(from, cls.globalServers[i].adr)) {
      CL_SetServerInfo(&cls.globalServers[i], info, ping);
    }
  }

  for (i = 0; i < MAX_OTHER_SERVERS; i++) {
    if (NETCompareAdr(from, cls.favoriteServers[i].adr)) {
...
void CL_ServerInfoPacket( netadr_t from, msg_t *msg ) {
  ...
  {
    if ( cl_pinglist[i].adr.port && !cl_pinglist[i].time && NETCompareAdr( from, cl_pinglist[i].adr ) )
    {
      // calc ping time
  ...
  Q_strncpyz( info, MSGReadString( msg ), MAX_INFO_STRING );
  if (strlen(info)) {
...
serverStatus_t *CL_GetServerStatus( netadr_t from ) {
  ...
  for (i = 0; i < MAX_SERVERSTATUSREQUESTS; i++) {
    if ( NETCompareAdr( from, cl_serverStatusList[i].address ) ) {
...
int CL_ServerStatus( char *serverAddress, char *serverStatusString, int maxLen ) {
  ...
  if ( NETCompareAdr( to, serverStatus->address) ) {
...
void CL_ServerStatusResponse( netadr_t from, msg_t *msg ) {
  ...
  for (i = 0, s = MSGReadStringLine( msg ); *s; s = MSGReadStringLine( msg ), i++) {
      ...
      if (server[i].visible) {
        if (server[i].ping == -1) {
          ...
          for (j = 0; j < MAX_PINGREQUESTS; j++) {
            ...
            if (NETCompareAdr( cl_pinglist[j].adr, server[i].adr)) {
              // already on the list
              break;
            }
          }
