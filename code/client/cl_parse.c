...
void CLParseSnapshot( msg_t *msg ) {
	...
	//clc.reliableAcknowledge = MSGReadLong( msg );
	...
	newSnap.serverTime = MSGReadLong( msg );

	newSnap.messageNum = clc.serverMessageSequence;

	deltaNum = MSGReadByte( msg );
	if ( !deltaNum ) {
		newSnap.deltaNum = -1;
	} else {
		newSnap.deltaNum = newSnap.messageNum - deltaNum;
	}
	newSnap.snapFlags = MSGReadByte( msg );
...
/*
=====================
CLParseCommandString
Command strings are just saved off until cgame asks for them
when it transitions a snapshot
=====================
*/
void CLParseCommandString(msg_t *msg) {
  char *s;
  int seq;
  int index;
...
/*
=====================
CLParseServerMessage
=====================
*/
void CLParseServerMessage(msg_t *msg) {
  int			cmd;
  ...
  while (1) {
    if (msg->readcount > msg->cursize) {
      Com_Error(ERR_DROP, "CLParseServerMessage: read past end of server message");
      break;
    }
    ...
    switch (cmd) {
    default:
      Com_Error(ERR_DROP, "CLParseServerMessage: Illegible server message\n");
      break;
    case svc_nop:
      break;
